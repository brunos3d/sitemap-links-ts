...
const fetchSitemap = (url: RequestInfo, timeout = 60000): Promise<any> => {
  ...
  return fetch(url, { signal, agent })
    .then((res) => res.text())
    .then((xml): any => {
      const { sitemapindex, urlset } = parser.parse(xml) as { sitemapindex?: { sitemap: SitemapURL | SitemapURL[] }; urlset?: SitemapURLSet };
